...
namespace QuartzServiceLib
{
    ...
    public abstract class QuartzServiceJob<T> : DataUtility, IQuartzServiceJob
    {
        ...
        public string GetLogFile()
        {
            ...
            return _log.Logger?.Repository?.GetAppenders()?.OfType<log4net.Appender.RollingFileAppender>()?.First()?.File?.Split(new string[] { ".log" },StringSplitOptions.RemoveEmptyEntries)?[0]?.ToString()+".log";
        }
