...
namespace AppUtilityLib
{
    ...
    public class AppUtility
    {
        ...
        {
            var rootAppender = LogManager.GetAllRepositories().Where(r => r.GetAppenders().OfType<RollingFileAppender>().Count()>0).First().GetAppenders().OfType<RollingFileAppender>().First();
        ...
        public int RunProcess(string Script, string Arguments, int? ScriptTimeOutInSeconds, out int PID,string WorkingFolder,out string StandardOutput,out string StandardError,ProcessWindowStyle WindowStyle= ProcessWindowStyle.Normal)
        {
            Process process = new Process();
            Logger.InfoFormat("RunProcess {0} {1}",Script,Arguments);
            System.Console.WriteLine("RunProcess {0} {1}",Script,Arguments);
